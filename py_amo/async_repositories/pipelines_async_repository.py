...
class PipelinesAsyncRepository(BaseAsyncRepository[PipelineSchema]):
    ...
    async def get_all_leads_count(self):
        ...
        result = await asyncio.gather(*(repository_safe_request(self.get_leads_count, semaphore, i, pipeline.id) for i, pipeline in enumerate(pipelines)))
        return sum(result)
    ...
    async def get_leads_count(self, pipeline_id: int):
        ...
        data = {
            'leads_by_status': 'Y',
            'skip_filter': 'Y',
            f'filter[pipe][{pipeline_id}][]': [str(status.id) for status in pipeline.statuses],
        }
        headers = {
            'accept': '*/*',
        ...
        response = await front_session.post(f"https://{self.subdomain}.amocrm.ru/ajax/leads/sum/{pipeline_id}/", data=data)
